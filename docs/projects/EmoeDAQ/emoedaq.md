...
EmoeDAQ是一款双通道、全差分输入的精密数据采集卡，具有±4.5V输入范围和24bit/32bit分辨率（24bit下，1 LSB等于0.6uV），同时具有极低的Noise Floor、极高的输入阻抗、低输入偏置电流，非常适合用于精密直流数据采集应用。
...
- 电源  
  - USB端口供电，电压为+5V±10%，单DAQ整机功耗约0.3W（5V 60mA）
  - 可从PMOD端口供电+通信（与USB二选一，二者间没有隔离），详见PMOD PIN MAP


## 通信与协议

- 通过USB-VCP（Virtual COM Port）与PC通信，用户也可自行从PMOD端与USART连接通信（需固件支持）
...
- 全差分输入测量范围为-4.5V至+4.5V（更常用的接法是将IN-接GND，IN+接输入电压，此时输入电压范围为0V至4.5V）
- 输入端极限过载电压为9V，内部设有气体放电管及熔丝用于过载保护（长时间过载将损坏前端，不建议在信号幅度超出输入范围的场合使用）
